...
impl From<AlgorithmArg> for Algorithm {
    fn from(a: AlgorithmArg) -> Self {
        match a {
            AlgorithmArg::AesEcb => Algorithm::AesEcb,
            AlgorithmArg::AesCtr => Algorithm::AesCtr,
            AlgorithmArg::CamelliaEcb => Algorithm::CamelliaEcb,
            AlgorithmArg::CamelliaCtr => Algorithm::CamelliaCtr,
            AlgorithmArg::ChaCha20 => Algorithm::ChaCha20,
        }
    }
}
...
mod test {
    ...
    const CHACHA20_PLAINTEXT: [u8; 128] = [
        ...
        0x6c, 0x64, 0x20, 0x62, 0x65, 0x20, 0x69, 0x74, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    const CHACHA20_IV: [u8; 16] = [
        0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x00,
        0x00,
    ];

    const CHACHA20_CIPHERTEXT: [u8; 128] = [
        ...
        0x0b, 0x8e, 0xed, 0xf2, 0x78, 0x5e, 0x42, 0x87, 0x4d, 0x74, 0x03, 0x73, 0x20, 0x1a, 0xa1,
    ...
    #[rstest]
    #[case::chacha20_encrypt(&CHACHA20_KEY, &CHACHA20_IV, Operation::Encrypt, &CHACHA20_CIPHERTEXT)]
    #[case::chacha20_decrypt(&CHACHA20_KEY, &CHACHA20_IV, Operation::Decrypt, &CHACHA20_CIPHERTEXT)]
    fn chacha20(
        #[case] key: &[u8],
        #[case] iv: &[u8],
        #[case] operation: Operation,
        #[case] ciphertext: &[u8; 128],
    ) {
        let request = CryptoRequest {
            algorithm: Algorithm::ChaCha20,
            operation,
        ...
        let (input, expected) = match operation {
            Operation::Encrypt => (CHACHA20_PLAINTEXT.as_ref(), ciphertext.as_ref()),
            Operation::Decrypt => (ciphertext.as_ref(), CHACHA20_PLAINTEXT.as_ref()),
            _ => unimplemented!(),
        };
        execute_crypto(&request, key, iv, input, &mut output);
        assert_eq!(expected, &output);
